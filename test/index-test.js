...
describe('index', () => {
	...
	describe('fetch functions', () => {
		...
		it('fetches the create fork api', async () => {
			...
			expect(
				window.fetch,
				'A fetch to the https://api.github.com/repos/learn-co-curriculum/js-ajax-fetch-lab/forks was not found'
			...
			expect(
				...
				'Authorization header expected to point to "token " without the actual token'
			).to.eq('token ');
		});
		...
		it('fetches the create issue api', async () => {
			...
			expect(
				window.fetch,
				`A POST request to the https://api.github.com/repos/${user}/js-ajax-fetch-lab/issues was not found`
			).to.have.been.called.with(`https://api.github.com/repos/${user}/js-ajax-fetch-lab/issues`);
			expect(
				window.fetch.__spy.calls[0][0],
				'Do not post issues to learn-co-curriculum, post them to your own fork'
			).to.not.match(/learn-co-curriculum/);
			expect(headers['authorization'], 'Authorization header not found').to.exist;
			expect(
				headers['authorization'][0],
				'Authorization header expected to point to "token " without the actual token'
		...
		it('fetches the get issues api', async () => {
			...
			expect(
				window.fetch,
				`A GET request to the https://api.github.com/repos/${user}/js-ajax-fetch-lab/issues was not found`
			).to.have.been.called.with(`https://api.github.com/repos/${user}/js-ajax-fetch-lab/issues`);
			expect(
				window.fetch.__spy.calls[0][0],
				'Do not get issues from the learn-co-curriculum repo, retrieve them from your own fork'
			).to.not.match(/learn-co-curriculum/);
			expect(headers['authorization'], 'Authorization header not found').to.exist;
			expect(
				headers['authorization'][0],
